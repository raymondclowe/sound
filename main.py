"""
Wake Word Detection POC (Proof of Concept)

This is a demonstration of the sound_recognition module capabilities.
It uses the module to:
1. Capture audio and detect silence
2. Match words using MFCC similarity
3. Transcribe audio using an external STT service
"""
...
# Import from the sound_recognition module
...
def main():
    ...
    if os.path.exists(REFERENCE_FILE) and not RECORD_REFERENCE:
        ...
        TARGET_WORD = transcribe_audio(ref_audio, SoundBuffer.FREQUENCY, stt_url=STT_URL, session=stt_session)
        if TARGET_WORD:
                ...
                if is_silent:
                    ...
                    if trailing_silence >= 1.0:
                        ...
                        if response == 'y':
                            ...
                            TARGET_WORD = transcribe_audio(reference_audio, SoundBuffer.FREQUENCY, stt_url=STT_URL, session=stt_session)
                            if TARGET_WORD:
                                # Clean up the transcription
                                TARGET_WORD = TARGET_WORD.strip().lower().rstrip('.,!?;:')
                                print(f"Target word: '{TARGET_WORD}'")
            ...
            if is_currently_silent:
                ...
                if trailing_silence_duration >= 0.5:  # Reduced from 1.0s to 0.5s
                    ...
                    if matches:
                        ...
                        transcription = transcribe_audio(word_audio, SoundBuffer.FREQUENCY, stt_url=STT_URL, prompt=prompt, model="tiny", session=stt_session)
